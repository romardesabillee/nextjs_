...
export default function Home() {
  ...
  useEffect(() => {
    authAxios.get(`${process.env.API_URL}/api/albums/`).then((resp) => {
      setAlbums(resp.data);
    })
    authAxios.get(`${process.env.API_URL}/api/albums/musicians/`).then((resp) => {
  ...
  function handleSubmit(e) {
    ...
    if(!id) {
      // create 
      authAxios.post(`${process.env.API_URL}/api/albums/`, {
      ...
      // update
      authAxios.put(`${process.env.API_URL}/api/albums/${id}/`, {
        artist: e.target.artist.value,
        name: e.target.name.value,
        date: e.target.date.value,
        stars: e.target.stars.value
  ...
  function handleDelete(id) {
    authAxios.delete(`${process.env.API_URL}/api/albums/${id}/`).then(() => {
